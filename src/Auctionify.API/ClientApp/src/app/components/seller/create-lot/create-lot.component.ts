...
export class CreateLotComponent {
  ...
  submitLot(isDraft: boolean) {
    ...
    this.client.createLot(lotToCreate).subscribe({
      next: (res) => {
        ...
        dialog.closed.subscribe(() => this.router.navigate(['/seller']));
      },
      error: (err) => {
  ...
  imageUpdateEvent(event: Event) {
    ...
    if (files.length > 1) {
      if (files.length > this.inputButtons.length - 1) {
        for (let i = this.inputButtons.length - 1; i < files.length; i++) {
  ...
  removeRemoveBtnFromImage(index: number) {
    const removeBtn = document.getElementById(`photo-button-remove-${index}`);
