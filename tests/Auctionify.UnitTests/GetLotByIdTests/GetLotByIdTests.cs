	...
	public class GetLotByIdTests
	{
		...
		public async Task GetByIdLotForBuyerQueryHandler_WhenCalledWithCorrectId_ReturnsLot()
		{
			...
			_watchListServiceMock
				.Setup(x => x.IsLotInUserWatchlist(It.IsAny<int>(), It.IsAny<int>(), default))
				.ReturnsAsync(false);

			// Lot without files
			var query = new GetByIdForBuyerLotQuery { Id = 2, };
		...
		public async Task GetByIdLotForBuyerQueryHandler_WhenCalledWithCorrectIdAndHasFiles_ReturnsLotWithFiles()
		{
			...
			// Lot with files
			var query = new GetByIdForBuyerLotQuery { Id = 1, };
		...
		public async Task GetByIdLotForBuyerQueryHandler_WhenCalledWithWrongId_ThrowsValidationException()
		{
			...
			// Lot with files
			var query = new GetByIdForBuyerLotQuery { Id = 0, };
			...
			await Assert.ThrowsAsync<ValidationException>(
				async () => await handler.Handle(query, default)
			);
		}
		...
		public async Task GetByIdLotForSellerQueryHandler_WhenCalledWithCorrectIdAndCreatedBySeller_ReturnsLot()
		{
			...
			var query = new GetByIdForSellerLotQuery { Id = 1, };
